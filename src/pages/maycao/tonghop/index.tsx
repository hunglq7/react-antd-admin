...
import { fetchDeleteTonghopmaycaoItem, fetchDeleteTonghopmaycaoItems, fetchTonghopmaycaoListWithPagination } from "#src/api/maycao/tonghop/index"
...
import { MaycaoModel } from "./components/Model"

export default function Tonghopmaycao() {
	const { t } = useTranslation()
	const { hasAccessByCodes } = useAccess()
	const [isOpen, setIsOpen] = useState(false)
	const [title, setTitle] = useState("")
	const [detailData, setDetailData] = useState<Partial<MaycaoTonghopItemType>>({})
	const [filteredData, setFilteredData] = useState<MaycaoTonghopItemType[]>([])
	const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([])
	const actionRef = useRef<ActionType>(null)

	const handleDeleteRow = async (id: number, action?: ProCoreActionType<object>) => {
		await fetchDeleteTonghopmaycaoItem(id)
		setSelectedRowKeys([])
		await action?.reload?.()
	...
	const handleBulkDelete = async () => {
		...
		await fetchDeleteTonghopmaycaoItems(selectedRowKeys as number[])
		setSelectedRowKeys([])
		await actionRef.current?.reload()
		window.$message?.success(t("common.deleteSuccess"))
	}

	const columns: ProColumns<MaycaoTonghopItemType>[] = [
	...
	return (
		<BasicContent className="h-full">
			<BasicTable<MaycaoTonghopItemType>
				adaptive
				columns={columns}
				actionRef={actionRef}
				rowSelection={{
				...
				request={async (params) => {
					...
					const result = await fetchTonghopmaycaoListWithPagination({
						keyword,
						pageIndex,
						pageSize,
					})
