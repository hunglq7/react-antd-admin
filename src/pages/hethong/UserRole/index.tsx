...
import { fetchDeleteUserRole, fetchDeleteUserRoleItems, fetchUserRoleList } from "#src/api/hethong/UserRole/index"
...
export default function UserRolePage() {
	...
	const [detailData, setDetailData] = useState<Partial<UserRoleItemType>>({})
	const [filteredData, setFilteredData] = useState<UserRoleItemType[]>([])
	const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([])
	const actionRef = useRef<ActionType>(null)

	const handleDeleteRow = async (record: UserRoleItemType, action?: ProCoreActionType<object>) => {
		await fetchDeleteUserRole(record)
		setSelectedRowKeys([])
		await action?.reload?.()
	...
	const handleBulkDelete = async () => {
		...
		const selectedItems = filteredData.filter((item) => selectedRowKeys.includes(`${item.userId}-${item.roleId}`))
		await fetchDeleteUserRoleItems(selectedItems)
		setSelectedRowKeys([])
	...
	// Tạo bảng dữ liệu
	const columns: ProColumns<UserRoleItemType>[] = [
		...
		<BasicContent className="h-full">
			<BasicTable<UserRoleItemType>
				...
				request={async (params) => {
					const data = await fetchUserRoleList()
					const filtered = data.filter((item) => {
						const keyword = String(params?.userName ?? "")
							.trim()
							.toLowerCase()

						return item.userName?.toLowerCase().includes(keyword) ?? false
					})
			...
			/>
			<Model title={title} open={isOpen} detailData={detailData} onCloseChange={onCloseChange} refreshTable={refreshTable} />
		</BasicContent>
	)
}
