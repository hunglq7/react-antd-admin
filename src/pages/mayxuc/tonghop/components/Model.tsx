...
export function Detail({
	...
	const [mayxucOptions, setMayxucOptions] = useState<
	...
	const onFinish = async (values: TonghopmayxucItemType) => {
		const selectedMayxuc = mayxucOptions.find(
			item => item.value === values.mayxucId,
		)
		...
		const loadOptions = async () => {
			...
			if (open) {
				form.setFieldsValue({
					...
					mayxucId:
						detailData.mayxucId
						?? mayxucOptions.find(item => item.label === detailData.tenMayXuc)
							?.value,
					phongBanId:
						detailData.phongBanId
						?? phongbanOptions.find(
							item => item.label === detailData.tenPhongBan,
						)?.value,
				})
			}
		}
		...
		>
			<Tabs
				...
				items={[
					...
					{
						label: "Nhật ký máy xúc",
						key: "nhatky",
						children: <NhatkyTab tonghopmayxucId={detailData.id} />,
					},
					{
						label: "Thông số máy xúc",
						key: "thongso",
						children: <ThongsoTab />,
					},
				]}
